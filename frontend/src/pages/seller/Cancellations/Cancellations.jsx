...
const Cancellations = () => {
    ...
    const fetchCancellations = async () => {
        setLoading(true);
        try {
            const response = await cancellationService.getAllCancellations({
                page: 1,
                limit: 1000,
                search: searchText
            });
            ...
            if (response?.data) {
                const cancellations = response.data.cancellations || response.data || [];
                ...
                cancellations.forEach(r => {
                    ...
                    if (st === 'REQUESTED' || st === 'PENDING') requested++;
                    else if (st === 'APPROVED') approved++;
                    else if (st === 'REJECTED') rejected++;
                    else if (st === 'COMPLETED') completed++;
                });
                setStats({ total: cancellations.length, requested, approved, rejected, completed });
            }
        } catch (error) {
            console.error(error);
            messageApi.error("Failed to load cancellation requests");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleView = async (record) => {
        setLoading(true);
        try {
            const response = await cancellationService.getCancellationById(record.id);
    ...
    const getStatusBadge = (status) => {
        ...
        if (s === 'REQUESTED' || s === 'PENDING') return <Badge status="warning" text="Requested" />;
    ...
    const getStatusTagColor = (status) => {
        const s = status ? status.toUpperCase() : '';
        if (s === 'REQUESTED' || s === 'PENDING') return 'orange';
    ...
    const columns = [
        {
            ...
            width: 200,
            render: (_, record) => (
                <Text strong>Cancel #{record.id?.substring(0, 13)}</Text>
            )
        },
        {
            title: "Customer",
        ...
        <>
            ...
            <div className="cancellations-page">
                <div className="page-header">
                    <Title level={2} style={{margin: 0, color: '#008ECC', fontWeight: '700'}}>Cancellation Requests</Title>
                </div>
                ...
                {selectedCancel && (
                    <div style={{paddingTop: 10}}>
                        <div className="cancel-modal-header">
                            <Title level={4} style={{margin:0}}>Cancellation Request</Title>
                            <Text type="secondary">ID: {selectedCancel.id}</Text>
                            <div style={{marginTop: 10}}>
                                {getStatusBadge(selectedCancel.status)}
                            </div>
                        </div>

                        <div className="cancel-section-title"><UserOutlined /> Customer Information</div>
                        <Descriptions size="small" bordered column={1}>
                            <Descriptions.Item label="Username">
                                {selectedCancel.Order?.Customer?.User?.username || "N/A"}
                            </Descriptions.Item>
                            <Descriptions.Item label="Email">
                                {selectedCancel.Order?.Customer?.User?.email || "N/A"}
                            </Descriptions.Item>
                        </Descriptions>

                        <div style={{marginTop: 16}}>
                            <div className="cancel-section-title" style={{marginBottom: 8}}>
                                <FileTextOutlined /> Cancellation Reason
                            </div>
                            <div style={{
                                padding: '12px',
                                backgroundColor: '#fff',
                                ...
                                {selectedCancel.reason}
                            </div>
                        </div>

                        <Divider dashed />

                        <div className="cancel-section-title"><CreditCardOutlined /> Order & Payment Info</div>
                        <Descriptions size="small" bordered column={2}>
                            <Descriptions.Item label="Order ID" span={2}>
                                <Text code copyable>{selectedCancel.Order?.id}</Text>
                            </Descriptions.Item>
                            <Descriptions.Item label="Order Date" span={2}>
                                {dayjs(selectedCancel.Order?.orderDate).format("DD/MM/YYYY HH:mm")}
                            </Descriptions.Item>

                            <Descriptions.Item label="Payment Method">
                                <Tag color="blue">{selectedCancel.Order?.Payment?.[0]?.method || "N/A"}</Tag>
                            </Descriptions.Item>
                            <Descriptions.Item label="Payment Status">
                                <Badge
                                    status={selectedCancel.Order?.Payment?.[0]?.status === 'COMPLETED' ? 'success' : 'warning'}
                                    text={selectedCancel.Order?.Payment?.[0]?.status}
                                />
                            </Descriptions.Item>
                            <Descriptions.Item label="Total Amount" span={2}>
                                <Text strong style={{fontSize: 16, color: '#008ECC'}}>
                                    {formatCurrency(selectedCancel.Order?.Payment?.[0]?.amount || 0)}
                                </Text>
                            </Descriptions.Item>
                        </Descriptions>
                        ...
                        <div className="cancel-section-title"><ShoppingOutlined /> Products in Order</div>
                        <div style={{maxHeight: 300, overflowY: 'auto', border: '1px solid #f0f0f0', borderRadius: 8, padding: '0 10px'}}>
                            <List
                                itemLayout="horizontal"
                                dataSource={selectedCancel.Order?.OrderItem || []}
                                renderItem={(item) => (
                                    <List.Item>
                                        <List.Item.Meta
                        ...
                        {(selectedCancel.status === 'REQUESTED' || selectedCancel.status === 'PENDING') && (
                            <div style={{marginTop: 20, textAlign: 'right'}}>
                                <Space>
                                    <Button danger onClick={() => {
                                        handleStatusChange(selectedCancel.id, 'REJECTED');
                                        setIsViewModalOpen(false);
                                    }}>Reject Request</Button>
                                    <Button type="primary" onClick={() => {
                                        handleStatusChange(selectedCancel.id, 'APPROVED');
                                        setIsViewModalOpen(false);
