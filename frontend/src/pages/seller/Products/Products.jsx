...
const Product = () => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            if (res?.data?.categories) setCategories(res.data.categories); else if (res?.data) setCategories(res.data);
        } catch (e) {
    ...
    },
    {
        ...
        render: (_, r) => {
            ...
            else if (getStockStatus(r.stockQuantity) === 'LOW_STOCK') return <Badge status="warning" text="Low Stock"/>
            else if (getStockStatus(r.stockQuantity) === 'OUT_STOCK') return <Badge status="error" text="Out Stock"/>
        }
    },
    {
        title: "Action",
    ...
    const renderProductFormContent = (formInstance) => {
    ...
    const tabItems = [
        { key: 'all', label: <span><ShopOutlined /> All Products</span> },
        { key: 'active', label: <span><CheckCircleOutlined /> Active</span> },
        { key: 'low_stock', label: <span><WarningOutlined /> Low Stock</span> },
        { key: 'out_stock', label: <span><InboxOutlined /> Out of Stock</span> },
    ];
    ...
    return (<>
            ...
            <Modal title="Edit Product"
                ...
                <Form form={form} layout="vertical">{renderProductFormContent(form)}</Form>
            </Modal>
